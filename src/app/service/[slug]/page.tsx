...
export default function ServicePage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 32, height: 32, borderRadius: '50%', border: '3px solid rgba(255,255,255,0.1)', borderTopColor: '#7c6af7', animation: 'spin 0.8s linear infinite' }} />
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <div className="svc-content" style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16, marginBottom: 32 }}>
          {service.target_user && (
            <InfoCard title="대상 사용자" content={service.target_user} icon="👤" />
          )}
          {service.key_features && (
            <InfoCard title="주요 기능" content={service.key_features} icon="✨" />
          )}
          {service.limitations && (
            <InfoCard title="제한 사항" content={service.limitations} icon="⚠️" />
          )}
